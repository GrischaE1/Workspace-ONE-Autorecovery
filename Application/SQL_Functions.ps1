<#
.SYNOPSIS
    Provides functions for interacting with an SQLite database.

.DESCRIPTION
    This file contains a collection of PowerShell functions that handle operations with an SQLite database.
    These functions support connecting to the database, executing queries, managing transactions, and performing
    common CRUD operations. They are used to log events, store state information, and maintain a historical record
    of health checks and recovery actions within the Workspace ONE Autorecovery solution.
...
.NOTES
    ...
    Usage        : Dot-source or import this file into your PowerShell session:
                    
                        . .\SQL_Functions.ps1
                   
                   Once imported, call the functions defined in this file to perform database operations.
...
function Insert-GeneralData {
    ...
    param(
        ...
        # A hashtable of key/value pairs to insert/update in the General table.
        [Parameter(Mandatory=$true)]
        [hashtable]$Data,
        
        # (Optional) A hashtable mapping keys (names) to their corresponding group.
        [Parameter(Mandatory=$false)]
    ...
    if (-not $Data -or $Data.Count -eq 0) {
        Write-Warning "Insert-GeneralData: No data provided. Skipping insertion."
        return
    }

    Write-Log "Bulk inserting/updating general data into the 'General' table." -Severity "INFO"

    # Loop through each key/value pair.
    foreach ($key in $Data.Keys) {
        # If the value is null or empty, assign the string "NULL"
        $value = $Data[$key]
        if ([string]::IsNullOrEmpty($value)) {
            $value = "NULL"
        }
        
        # Build a record with the columns 'Name' and 'Value'
        $record = @{
            Name  = $key
            Value = $value
        }
        
        # If a group mapping was provided and contains this key, add the group.
        if ($GroupMapping -and $GroupMapping.ContainsKey($key)) {
            $record["Group"] = $GroupMapping[$key]
        }

        # Insert/overwrite the record in the General table.
