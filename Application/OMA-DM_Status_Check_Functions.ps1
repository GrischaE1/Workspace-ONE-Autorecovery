<#
.SYNOPSIS
    Provides functions to check the status and connectivity of the OMA-DM service.

.DESCRIPTION
    This file contains a collection of PowerShell functions that perform health and connectivity checks for the 
    OMA-DM (Open Mobile Alliance Device Management) service. These functions are critical in ensuring that the mobile 
    device management capabilities within Workspace ONE are operational and properly configured. They help monitor 
    service responsiveness and identify potential issues with the OMA-DM endpoints.
...
.NOTES
    ...
    Usage        : This file is not intended to be run on its own. Instead, dot-source or import it into your main 
                   scripts as follows:
                   
                        . .\OMA-DM_Status_Check_Functions.ps1
                   
                   Once imported, the functions defined in this module can be used to perform OMA-DM status checks 
                   within your monitoring and recovery workflows.
